package com.face.ethlinstener.ui;
   ...
   private static int TIMEOUT = 4000; // TIMEOUT
    ...
    public static NET_TYPE getNetState(Context context, String urlAddr) {
        try {
            ...
            e.printStackTrace();
            Log.e(TAG,"getNetState() errMeg:"+e.getMessage());
        }
        //网络未准备好
        //需要再次确认网络是否正常
        if(isAgainConn(urlAddr)){
    ...
    public static boolean isAgainConn(String url) {
        ...
        //Log.e(TAG,"网络出现问题,正在调整访问地址以进行网络判断");
        if (url.equals(publicUrl[0]) || url.equals(publicUrl[1])) {
            ...
            //Log.e(TAG,"-经过再次访问,确定了网络是否正常："+isAgain);
            return isAgain;
        } else {
            isAgain = pingIp(publicUrl[0]);
            if(isAgain){
                //Log.e(TAG,"--经过再次访问,确定了网络是否正常："+true);
                return true;
            }else{
                //Log.e(TAG,"--经过再次访问,确定了网络是否正常："+false);
                isAgain=  pingIp(publicUrl[1]);
                //Log.e(TAG,"---经过再次访问,确定了网络是否正常："+isAgain);
                return isAgain;
            }
